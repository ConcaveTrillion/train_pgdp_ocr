...
class NiceGuiLineEditor:
    ...
    def draw_ui(self, parent_container):
        ...
        with parent_container:
            with ui.card().style(
                f"margin: 5px; padding: 5px; {self.get_border_style()}"
            ):
    ...
    def draw_line_image(self):
        ...
        with ui.row(wrap=False).classes("w-full h-[40px]"):
            if (
                ...
                x1, y1, x2, y2 = (
                    int(scaled_bbox.minX),
                    int(scaled_bbox.minY),
                    int(scaled_bbox.maxX),
                    int(scaled_bbox.maxY),
                )
                ...
                self.line_image = ui.interactive_image(
    ...
    def draw_action_buttons(self):
        ...
        with ui.row().classes("gap-2"):
            ...
            ui.button("Delete Line", on_click=self.delete_line).props(
                "size=sm color=negative"
            )
            ui.button("Mark as Validated", on_click=self.mark_validated).props(
                "size=sm color=positive"
            )

            ui.label("Crop:").classes("self-center")
            ui.button("T", on_click=lambda: self.crop_words("T")).props(
                "size=sm"
            ).style("width: 30px")
            ui.button("B", on_click=lambda: self.crop_words("B")).props(
                "size=sm"
            ).style("width: 30px")
            ui.button("A", on_click=lambda: self.crop_words("A")).props(
                "size=sm"
            ).style("width: 30px")
    ...
    def draw_word_match_card(self, match):
        ...
        with ui.card().tight():
            with ui.column():
                with ui.row(wrap=False).classes("w-full h-[40px]"):
                    # Word image
                    data_src = match.get("data_src_string")
                    if data_src:
                        # The data_src_string should already include the data: prefix
                        ui.interactive_image(data_src).classes("h-full")
                    else:
                        ui.label("No Image").classes("text-xs text-red-500")
                # OCR text
                ocr_color = match.get("ocr_text_color", "black")
                ocr_text = match.get("ocr_text", "No OCR")
                ui.label(ocr_text).classes("text-xs font-mono text-center").style(
                    f"color: {ocr_color}; word-break: break-all;"
                )

                # Ground truth text (editable)
                gt_text = match.get("gt_text", "")

                def update_handler(e, current_match=match):
                    self.update_gt_text(e.sender.value, current_match)

                gt_input = (
                    ui.input(value=gt_text, placeholder="Ground truth")
                    .classes("text-xs font-mono")
                    .style("width: 100%;")
                )
                gt_input.on("blur", update_handler)

                # Action buttons
                if self.task_type == EditorTaskType.NONE:
                    with ui.row().classes("gap-0 justify-center"):
                        ui.button(
                            "X", on_click=lambda _=None, m=match: self.delete_match(m)
                        ).props("size=xs color=negative").style("width: 20px")
                    ...
                    with ui.row().classes("gap-0 justify-center"):
                        ...
                        if word:
                            if match.get("word_idx", 0) > 0:
                                ui.button(
                                    "ML",
                                    on_click=lambda _=None, m=match: self.merge_left(m),
                                ).props("size=xs").style("width: 25px")

                            if (
                                match.get("word_idx", 0)
                                < len(self._current_ocr_line.items) - 1
                            ):
                                ui.button(
                                    "MR",
                                    on_click=lambda _=None, m=match: self.merge_right(
                                        m
                                    ),
                                ).props("size=xs").style("width: 25px")
                        ...
                        with ui.row().classes("gap-0 justify-center"):
                            ui.button(
                                ...
                                m=match: self.start_edit_bbox_task(m),
                            ).props("size=xs").style("width: 25px")
                            ui.button(
                                "SP",
                                on_click=lambda _=None, m=match: self.start_split_task(
                                    m
                                ),
                            ).props("size=xs").style("width: 25px")
                    ...
                    if word:
                        with ui.row().classes("gap-0 mt-1"):
                            ui.button(
                                "CT",
                                on_click=lambda _=None, m=match: self.crop_word_top(m),
                            ).props("size=xs").style("width: 25px")
                            ui.button(
                                "CB",
                                on_click=lambda _=None, m=match: self.crop_word_bottom(
                                    m
                                ),
                            ).props("size=xs").style("width: 25px")
                            ui.button(
                                "CA",
                                on_click=lambda _=None, m=match: self.crop_word_both(m),
                            ).props("size=xs").style("width: 25px")
    ...
    def draw_split_task_ui(self):
        ...
        with ui.row().classes("gap-2 mt-2"):
            ui.button("Cancel", on_click=self.cancel_split_task).props(
                "color=negative size=sm"
            )
            ui.button("<1p", on_click=lambda: self.split_move_pixels(-1)).props(
                "size=sm"
            ).style("width: 35px")
            ui.button(">1p", on_click=lambda: self.split_move_pixels(1)).props(
                "size=sm"
            ).style("width: 35px")
            ui.button("<5%", on_click=lambda: self.split_move_percent(-0.05)).props(
                "size=sm"
            ).style("width: 35px")
            ui.button(">5%", on_click=lambda: self.split_move_percent(0.05)).props(
                "size=sm"
            ).style("width: 35px")
            ui.button("<20%", on_click=lambda: self.split_move_percent(-0.2)).props(
                "size=sm"
            ).style("width: 40px")
            ui.button(">20%", on_click=lambda: self.split_move_percent(0.2)).props(
                "size=sm"
            ).style("width: 40px")
            ui.button("Execute Split", on_click=self.execute_split).props(
        ...
        ui.html(
            f"<span style='font-family: {self.monospace_font_name}; font-size: 16px;'>{split_text}</span>"
        )

        with ui.row().classes("gap-2 mt-1"):
            ui.button("<", on_click=lambda: self.split_move_text(-1)).props(
                "size=sm"
            ).style("width: 25px")
            ui.button(">", on_click=lambda: self.split_move_text(1)).props(
                "size=sm"
    ...
    def draw_edit_bbox_task_ui(self):
        ...
        ui.label("Edit Bounding Box Task Active").classes(
        ...
        with ui.row().classes("gap-2 mt-2"):
            ui.button("Cancel", on_click=self.cancel_edit_bbox_task).props(
                "color=negative size=sm"
            )
            ui.button("Refine", on_click=self.edit_bbox_refine).props("size=sm")
            ui.button("Save", on_click=self.execute_edit_bbox_task).props(
                "color=positive size=sm"
            )
        ...
        with ui.grid(columns=3).classes("gap-2 w-fit"):
            ...
            with ui.column().classes("items-center"):
                ...
                with ui.row().classes("gap-1"):
                    ui.button(
                        "↑1", on_click=lambda: self.edit_bbox_adjust_margin("T", -1)
                    ).props("size=xs")
                    ui.button(
                        "↑5", on_click=lambda: self.edit_bbox_adjust_margin("T", -5)
                    ).props("size=xs")
                    ui.button(
                        "↑10", on_click=lambda: self.edit_bbox_adjust_margin("T", -10)
                    ).props("size=xs")
                with ui.row().classes("gap-1"):
                    ui.button(
                        "↓1", on_click=lambda: self.edit_bbox_adjust_margin("T", 1)
            ...
            with ui.column().classes("items-center"):
                ...
                with ui.row().classes("gap-1"):
                    ui.button(
                        "←1", on_click=lambda: self.edit_bbox_adjust_margin("L", -1)
                    ).props("size=xs")
                    ui.button(
                        "←5", on_click=lambda: self.edit_bbox_adjust_margin("L", -5)
                    ).props("size=xs")
                    ui.button(
                        "←10", on_click=lambda: self.edit_bbox_adjust_margin("L", -10)
                    ).props("size=xs")
                with ui.row().classes("gap-1"):
                    ui.button(
                        "→1", on_click=lambda: self.edit_bbox_adjust_margin("L", 1)
            ...
            with ui.column().classes("items-center"):
                ...
                with ui.row().classes("gap-1"):
                    ui.button(
                        "←1", on_click=lambda: self.edit_bbox_adjust_margin("R", -1)
                    ).props("size=xs")
                    ui.button(
                        "←5", on_click=lambda: self.edit_bbox_adjust_margin("R", -5)
                    ).props("size=xs")
                    ui.button(
                        "←10", on_click=lambda: self.edit_bbox_adjust_margin("R", -10)
                    ).props("size=xs")
                with ui.row().classes("gap-1"):
                    ui.button(
                        "→1", on_click=lambda: self.edit_bbox_adjust_margin("R", 1)
            ...
            with ui.column().classes("items-center"):
                ...
                with ui.row().classes("gap-1"):
                    ui.button(
                        "↑1", on_click=lambda: self.edit_bbox_adjust_margin("B", -1)
                    ).props("size=xs")
                    ui.button(
                        "↑5", on_click=lambda: self.edit_bbox_adjust_margin("B", -5)
                    ).props("size=xs")
                    ui.button(
                        "↑10", on_click=lambda: self.edit_bbox_adjust_margin("B", -10)
                    ).props("size=xs")
                with ui.row().classes("gap-1"):
                    ui.button(
                        "↓1", on_click=lambda: self.edit_bbox_adjust_margin("B", 1)
    ...
    def split_move_pixels(self, amount: int):
        ...
        self.split_task_x_coordinate = max(
            0, min(w, self.split_task_x_coordinate + amount)
        )
        self.draw_active_task()
    ...
    def split_move_percent(self, amount: float):
        ...
        self.split_task_x_coordinate = max(
            0, min(w, self.split_task_x_coordinate + pixel_amount)
        )
        self.draw_active_task()
    ...
    def split_move_text(self, amount: int):
        ...
        match = self.line_matches[self.task_match_idx]
        ocr_text = match.get("ocr_text", "")
        self.split_task_word_split_idx = max(
            0, min(len(ocr_text), self.split_task_word_split_idx + amount)
        )
        self.draw_active_task()
